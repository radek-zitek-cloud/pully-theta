...
func TestHTTPErrorMapper_NewHTTPErrorMapper(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mapper := NewHTTPErrorMapper(tt.logger)

			assert.NotNil(t, mapper, "Error mapper should not be nil")
			assert.NotNil(t, mapper.logger, "Logger should not be nil")

			if tt.logger != nil {
				assert.Equal(t, tt.logger, mapper.logger, "Should use provided logger")
			}
		})
	}
}
...
func TestHTTPErrorMapper_MapError(t *testing.T) {
	...
	mapper := NewHTTPErrorMapper(logger)
...
// TestHTTPErrorMapper_CategorizeError tests the internal error categorization logic.
func TestHTTPErrorMapper_CategorizeError(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			httpCode, errorCode, _, _ := mapper.categorizeError(tt.error)
			assert.Equal(t, tt.expectedCode, httpCode)
			assert.Equal(t, tt.expectedType, errorCode)
		})
	}
}
